...
import DataTable from "../shared-directory/components/DataTable";
import DataItem from "../shared-directory/components/DataItem";
import TableDataViewModal from "../shared-directory/components/TableDataViewModal";
import FileViewModal from "../shared-directory/components/FileViewModal";
...
export default class extends React.Component{
  ...
  componentWillMount(){
    ...
    const { data = [] } = this.props.sharedResource;
    let dataItem = {};
    ...
    if (dataItem.database && dataItem.database.tables && dataItem.database.tables.length > 0) {
      payload.currentLabel.push('database');
      payload.tabs = this.state.tabs.filter(tab => !['interface','fileDownload'].includes(tab.key))
    } else if (dataItem.interface) {
      payload.currentLabel.push('interface');
      payload.tabs = this.state.tabs.filter(tab => !['table','fileDownload'].includes(tab.key))
    } else{
      payload.currentLabel.push('file');
      payload.tabs = this.state.tabs.filter(tab => !['dataItem','dataDetail','table','interface','message'].includes(tab.key))
    }
    if (dataItem.folders && dataItem.folders.length > 0) {
      payload.currentLabel.push('folder');
    }
    if (tables.length > 0) {
  ...
  updateSubscription = () => {
    ...
    this.props.dispatch({
      type: 'sharedResource/updateSubscription',
      payload: {id}
    })
  };
        ...
        <div className={styles.top}>
          <div className={styles.left}>
            ...
            <div className={styles.number}>
              {this.state.currentLabel.includes('database') && <Button className={styles.btn_1}>库表</Button>}
              {this.state.currentLabel.includes('folder') && <Button className={styles.btn_folder}>文件夹</Button>}
              {this.state.currentLabel.includes('file') && <Button className={styles.btn_2}>文件</Button>}
              {this.state.currentLabel.includes('interface') && <Button className={styles.btn_3}>接口</Button>}
